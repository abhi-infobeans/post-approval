...
/**
 * Function to check post exists or not
 * 
 * @return int count
 */
...
/**
 * Function to check editor posts and assign
 * 
 * @return Review User
 * 
 * @param  Post ID
...
/**
 * Function Ajax request to delete a restricted post
 * 
 * @return message
 * 
 * @param  Post ID
...
/**
 * Function to re-assign post to the editor.
 *
...
/**
 * Function to get editor to assign post 
 * 
 * @param 
 * @return post array
 */
...
{
	global $wpdb;
	$all_ids =  $args = $review_posts = array();
...
/**
 * Function to get all assigned post for Amdin 
 * 
 * @param 
 * @return post array
 */
 ...
 * @return post array
 */
function ajax_get_all_review_post()
{
	global $wpdb;
	$all_ids =  $args = $review_posts = array();
	$login_user = get_current_user_id();
	$user_approval_table = $wpdb->prefix . "user_approval_post";
	if ($login_user) {
		$review_post_ids = $wpdb->get_results("SELECT id, post_id FROM $user_approval_table limit 5", ARRAY_A);
		if ($review_post_ids) {
			foreach ($review_post_ids as $ids) {
				$all_ids[] = (int) $ids['post_id'];
			}
	...
	foreach ($all_ids as $ids) {
		...
		if (get_post_status($ids) == 'publish') {
			$wpdb->delete(TABLE_USER_POST_APPROVAL, array('post_id' => $ids));
			delete_post_meta($post['ID'], 'post_viewer');
			$wpdb->query($sql);
			echo json_encode(array('success' => true, 'id' => $ids));
		} else {
