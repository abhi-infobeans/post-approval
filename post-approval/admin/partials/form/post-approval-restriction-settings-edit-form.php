    ...
    if(isset($_POST['savedata']) && $_POST['savedata'] == 'savedata' && $_POST['restricted_post']!=''){
     ...
     <script>function updateurl(){ window.location.href = "'.$url.'admin.php?page=restricted-post-list";} 
	  ...
	  echo '<div class="wrap"><div id="icon-tools" class="icon32"></div>';
		...
		echo '<h1 class="wp-heading-inline">Edit Restriction Settings</h1><br><br>';
  ...
  echo '<div><b>Please select the type of post that needs to be restricted </b></div><br>';
	...
	if ( $post_types ) { // If there are any custom public post types.
      ...
      echo '<option value="">Please select post type</option>';
        foreach ( $post_types  as $post_type ) {
          $selected = ($edit_post_type == $post_type) ? 'selected' : '';
    ...
    echo '<div><br><b>Please select the user that you want to restrict as a viewer of the post </b></div><br>';
		...
		if ( $usersData ) { // If there are any editor users.
   		...
   		if($edit_post_type){
						    	$key=$edit_post_type."_post_restricted_users";
						      ...
						      echo '<option  value ='.$user->id.' '.$checked_val.' >' . ucfirst($user->display_name  ).'</option>';
								  }
			}else{
                  foreach ( $usersData  as $user ) {
